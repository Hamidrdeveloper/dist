...
const settingsAPI = new ApiBuilder<SaleSystemPure>('/settings', i18n.t('ShopSettings.Tab.SaleSystem'));

export const getSaleSystemSettings = async (): Promise<SaleSystemModel | null> => {
  try {
...
export const editSaleSystemSettings = async (data: SaleSystemPure): Promise<SaleSystemModel | null> => {
  try {
    ...
    message.success(i18n.t('Global.UpdatedSuccessfully', { title: i18n.t('ShopSettings.Tab.SaleSystem') }));
    return result;
  } catch (e) {
    throw new Error(e);
  }
