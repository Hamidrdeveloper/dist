import i18n from '@src/core/i18n/config';
import { PageLayout } from '@src/shared/components';
import ModalHeader from '@src/shared/components/PageLayout/GenericModal/ModalHeader/ModalHeader';
import { UpsertWrapper } from '@src/shared/styles';
import React, { ReactElement, useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';

import TemplateUpsert from '../containers/TemplateUpsert';
import { Template } from '../model/template.entity';
import moduleInfo from '../ModuleInfo.json';
import TemplateModule from '../Template.Module';

const TemplateUpsertPage = (): ReactElement => {
  const navigate = useNavigate();
  const { template_id: id } = useParams();
  const [singleData, setSingleData] = useState({} as Template);
  const templateModule = new TemplateModule();
  const title = templateModule.title[0];
  const moduleUrl = `${moduleInfo.Route.replace('*', '')}`;

  const routes = [
    ...templateModule.breadcrumbItems,
    {
      path: '',
      breadcrumbName: id ? `${i18n.t('Global.Update')} ${title} - ${id}` : `${i18n.t('Global.New')} ${title}`,
    },
  ];

  useEffect(() => {
    if (id) {
      templateModule.apiService.getOne(+id).then((data) => {
        setSingleData(data);
      });
    }
  }, []);

  const goBack = () => {
    navigate(-1);
  };
  return (
    <PageLayout<Template> module={templateModule}>
      <PageLayout.Breadcrumb routes={routes} />

      <PageLayout.Panel noOverflow>
        <UpsertWrapper>
          <div className="header">
            <ModalHeader onClose={() => navigate(`/admin${moduleUrl}`)} items={routes} />
          </div>
          <TemplateUpsert singleData={singleData} onCallback={goBack} />
        </UpsertWrapper>
      </PageLayout.Panel>
    </PageLayout>
  );
};

export default TemplateUpsertPage;
