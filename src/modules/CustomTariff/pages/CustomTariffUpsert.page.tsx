...
import CustomTariffUpsert from '../containers/CustomTariffUpsert';
import CustomTariffModule from '../CustomTariff.module';
import { CustomTariff } from '../model/customTariff.entity';
import moduleInfo from '../ModuleInfo.json';

const CustomTariffUpsertPage = (): ReactElement => {
  const navigate = useNavigate();
  const { customTariff_id: id } = useParams();
  const [singleData, setSingleData] = useState({} as CustomTariff);
  const [isLoading, setIsLoading] = useState(false);
  const customTariffModule = new CustomTariffModule();
  const title = customTariffModule.title[0];
  const moduleUrl = `${moduleInfo.Route.replace('*', '')}`;

  const routes = [
    ...customTariffModule.breadcrumbItems,
    {
      path: '',
    ...
    <PageLayout<CustomTariff> module={customTariffModule}>
      ...
      <PageLayout.Panel>
        <UpsertWrapper>
          ...
          {isLoading ? <Loader /> : <CustomTariffUpsert singleData={singleData} onCallback={goBack} />}
        </UpsertWrapper>
      </PageLayout.Panel>
    </PageLayout>
  );
...
export default CustomTariffUpsertPage;
