...
const SubdomainSocialMedia = ({
  ...
  const [settings, setSettings] = useState<SocialMediaModel[]>([]);

  useEffect(() => {
    setSettings(socialMediaData);
  }, []);
...
export default SubdomainSocialMedia;

type handleFormSubmitArguments = {
  data: SocialMediaModel[];
  states: { setPending: (pending: boolean) => void; setSettings: (settings: SocialMediaModel[]) => void };
  partner_id: number;
  subdomain_id: number;
  company_id?: number;
};
...
const handleFormSubmit = ({
  ...
  setPending(true);
  editSocialMediaSetting(data, subdomain_id, partner_id, company_id)
    .then((res) => {
      setPending(false);
      res && setSettings(res);
    })
