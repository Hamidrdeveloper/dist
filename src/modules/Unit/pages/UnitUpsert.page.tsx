...
import UnitUpsert from '../containers/UnitUpsert';
import { Unit } from '../model/unit.entity';
import moduleInfo from '../ModuleInfo.json';
import UnitModule from '../Unit.module';

const UnitUpsertPage = (): ReactElement => {
  const navigate = useNavigate();
  const { unit_id: id } = useParams();
  const [singleData, setSingleData] = useState({} as Unit);
  const unitModule = new UnitModule();
  const title = unitModule.title[0];
  const moduleUrl = `${moduleInfo.Route.replace('*', '')}`;

  const routes = [
    ...unitModule.breadcrumbItems,
    {
      path: '',
  ...
  useEffect(() => {
    if (id) {
      unitModule.apiService.getOne(+id).then((data) => {
        setSingleData(data);
      });
    }
  }, []);
    ...
    <PageLayout<Unit> module={unitModule}>
      ...
      <PageLayout.Panel>
        <UpsertWrapper>
          ...
          <UnitUpsert singleData={singleData} onCallback={goBack} />
        </UpsertWrapper>
      </PageLayout.Panel>
    </PageLayout>
  );
