...
import { BillDescColumns } from '../components/BillsDescTableInfo';
import { Bill } from '../model/bill.entity';
import { getAllBills } from '../service/bills.service';
...
interface HooksType {
  ...
  onSortChange: (p: TablePaginationConfig, f: Record<string, any>, sorter: SorterResult<Bill>) => void;
}

export function useLoadItems(aBillId?: string): HooksType {
  ...
  const [tableColumns, setTableColumns] = useState<ColumnsType<Bill>>([]);
  ...
  const onSortChange = (p: TablePaginationConfig, f: Record<string, any>, sorter: SorterResult<Bill>) => {
  ...
  async function loadBills(pageNumber?: number, isResetFilters = false, search = '') {
    ...
    try {
      ...
      const { data, meta } = await getAllBills({
        page: pageNumber ?? page,
        params: { ...reqParams, abillId: Number(aBillId) },
      });
      ...
      if (currentPage === 1) {
        ...
        if (Object.keys(orderByField).length === 0) {
          const defaultBillsTableColumns = BillDescColumns(+(aBillId ?? 0));

          setTableColumns(
            defaultBillsTableColumns.map((column) => {
  ...
  function normalizeValuesOfPerformedFilters(performedFilters: Record<string, any>): Record<string, unknown> {
    for (const key in performedFilters) {
      if (performedFilters[key].value !== undefined) {
        performedFilters[key] = performedFilters[key].value;
        console.log({ performedFilters, key, no: false });
      } else {
        console.log({ performedFilters, key, no: true });
      }
    }
